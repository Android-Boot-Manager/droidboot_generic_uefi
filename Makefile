...
export EDK2BASE=../edk2
# NOTE: Below is overridden for "tiano" targets
-include $(EDK2BASE)/Conf/target.txt
THISDIR=$(shell pwd)
EDK2_BUILDLOC=$(EDK2BASE)/Build/Abm/$(TARGET)_$(TOOL_CHAIN_TAG)/$(UC_ARCH)
EDK2_PROGRAM_BASENAMES=abm gptsync
...
tiano:
	...
	+make MAKEWITH=TIANO BUILDME=abm DLL_TARGET=abm -C $(LOADER_DIR) -f Make.tiano
ifneq ($(ARCH),aarch64)
...
edk2: build_edk2
	cp $(EDK2_BUILDLOC)/abm.efi ./abm/abm_$(FILENAME_CODE).efi
...
build_edk2: $(EDK2BASE)/AbmPkg
	cd $(EDK2BASE) && \
	. ./edksetup.sh BaseTools && \
	build -a $(UC_ARCH) -p AbmPkg/AbmPkg.dsc
	mkdir -p ./drivers_$(FILENAME_CODE)

$(EDK2BASE)/AbmPkg:
	ln -s $(THISDIR) $(EDK2BASE)/AbmPkg
...
clean:
	...
	rm -f include/*~
	rm -rf $(EDK2BASE)/Build/Abm
	rm -rf drivers_$(FILENAME_CODE)/*
	[ ! -L $(EDK2BASE)/AbmPkg ] || rm -v $(EDK2BASE)/AbmPkg
